...
def get_map_counters(interface):
    try:
        prog_id = get_xdp_prog(interface)
        cmd_progshow = "bpftool prog show id %d -p" % prog_id
        prog_info = check_output_json(cmd_progshow)
        ...
        for m in maps:
            if m["name"] == "counters":
                if m["id"] in prog_info["map_ids"]:
                    map_ids = get_map_counters_entries(m["id"])
        return map_ids
    except Exception as e:
        return str(e)


def get_map_counters_entries(map_id):
    try:
        cmd_progshow = "bpftool map dump id %d -j" % map_id
        total_entries = check_output_json(cmd_progshow)
        entries = []
        for n in total_entries:
            src_ip = [int(byte.replace("0x", ""), 16) for byte in n["key"][0:4]]
            ip_src = "%s.%s.%s.%s" % (src_ip[0], src_ip[1], src_ip[2], src_ip[3])

            dest_ip = [int(byte.replace("0x", ""), 16) for byte in n["key"][4:8]]
            ip_dst = "%s.%s.%s.%s" % (dest_ip[0], dest_ip[1], dest_ip[2], dest_ip[3])

            lbl_hex = n["key"][8:12]
            lbl_hex_full = "".join(x.replace("0x", "") for x in lbl_hex)
            lbl = int(lbl_hex_full, 16)

            pkts = hex_list_to_int(n["value"][0:8])
            bbytes = hex_list_to_int(n["value"][8:15])
...
def update_map(map_id, subnet, lbl, action="add"):
    ...
    ip_hex = "0x{:02X} 0x{:02X} 0x{:02X} 0x{:02X}".format(
